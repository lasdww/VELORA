...
[![Engines](https://img.shields.io/badge/Engines-Sing--Box%20%7C%20Xray%20Core-00B4D8?style=for-the-badge)](https://github.com/lasdww/VELORA)
...
<table>
  <tr>
    ...
    <td align="center" width="50%">
      ...
      <br/><br/>
      <sub>Размер: ~100 МБ · Включает все компоненты и драйвер Wintun внутри</sub>
    </td>
  </tr>
</table>
...
[⚡ О проекте](#-о-проекте) • [🛡️ Возможности](#-ключевые-возможности) • [🎮 Игровой фильтр](#-игровой-фильтр) • [⌨️ Горячие клавиши](#️-горячие-клавиши) • [⚙️ Архитектура](#-архитектура-и-стек) • [💻 Требования](#-системные-требования)
...
**Velora** — это высокоскоростной сетевой клиент нового поколения для Windows, объединяющий современный стек протоколов **VLESS Reality (XTLS Vision)** и встроенный комплекс прямого ускорения и обхода блокировок **Fix Internet (DPI Bypass)** в едином стильном приложении.

Проект создан с упором на максимальную скорость без задержек, бескомпромиссную конфиденциальность и визуальное совершенство в тёмном обсидиановом стиле с аппаратными анимациями.

---

## 🛡️ Ключевые возможности

### 1. Двойной сетевой стек: VLESS Reality + Fix Internet
* **VLESS Reality & XTLS Vision**:
  * Под капотом используются оптимизированные высокопроизводительные ядра **Sing-Box 1.10+** и **Xray-Core**.
  * Поддержка браузерной маскировки **uTLS Fingerprint** (Chrome, Firefox, Safari, Edge) для полного сокрытия VPN-трафика от алгоритмов анализа пакетов.
...
* **Fix Internet (DPI Bypass)**:
  * Прямой обход блокировок без необходимости подключаться к внешним VPN-серверам! Доступ к YouTube в 4K 60fps, Discord, Twitch и другим ресурсам на полной скорости вашего домашнего провайдера.
  * **20 предустановленных стратегий** прямого обхода DPI (модификация TCP/UDP заголовков, фрагментация, fake TLS ClientHello).
  * Встроенный каталог из **70+ популярных сервисов** с категоризацией, мгновенным поиском и возможностью добавлять свои домены.

### 2. Игровой фильтр (Game UDP Filter)
* Интегрированная кнопка «Игровой фильтр» с аппаратным переключателем.
* Автоматически исключает UDP-трафик онлайн-игр и голосовых чатов (порты `1024-65535`) из обработки DPI-фильтром, гарантируя нулевой дополнительный пинг и отсутствие потерь пакетов в играх (CS2, Dota 2, Valorant, Apex Legends) и Discord Voice.

### 3. Сетевой адаптер Wintun и умная маршрутизация
* **Wintun TUN Adapter**:
  * Полноценный виртуальный сетевой интерфейс уровня ядра Windows.
  * Трафик всех программ, браузеров, игр и системных служб направляется через туннель без необходимости ручной настройки системного прокси.
* **Раздельное туннелирование (Split Tunneling)**:
  * **[Весь трафик]** — глобальное туннелирование.
  * **[В обход сайтов РФ]** — российские сервисы (Яндекс, Госуслуги, VK, банки, онлайн-кинотеатры) идут напрямую на гигабитной скорости, а заблокированные зарубежные ресурсы — через туннель.
  * **[Только заблокированные]** — туннелирование только ресурсов из списка ограничений.
...
* **8 неоновых палитр**:
  ...
  * 🍋 Лайм (`#84CC16`)
  * ⚙️ Титан (`#94A3B8`)
* **Консольный терминал логов**: терминал в стиле macOS с цветными статус-точками, меткой `tunnel.log`, автоскроллом и живой трансляцией сетевых событий и подключения.
...
* Никаких сторонних баз данных, облачных телеметрий или скрытых запросов.
...
2. **Запустите Velora от имени администратора**:
   > [!IMPORTANT]
   > Права администратора требуются для создания виртуального сетевого адаптера `Wintun` и работы низкоуровневого драйвера пакетов `WinDivert`.
3. **Добавьте сервер**:
   * Нажмите `Ctrl + V` для мгновенной вставки VLESS-ключа или ссылки на подписку из буфера обмена.
   * Либо перейдите во вкладку **«Обход блокировок»** и включите тумблер **«Fix Internet»** для ускорения YouTube и Discord без сторонних серверов.
4. **Подключитесь**:
...
| **DPI Engine** | Модуль обхода ТСПУ/DPI методом модификации TCP/UDP заголовков |
...
* **Права доступа**: Права локального администратора (для создания адаптера Wintun и запуска драйвера WinDivert).
